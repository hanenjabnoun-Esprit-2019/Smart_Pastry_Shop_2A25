...
QSqlQueryModel*Ingredient::afficher()
{
    QSqlQueryModel*model=new QSqlQueryModel();
         ...
         model->setHeaderData(1, Qt::Horizontal, QObject::tr("Nom d'ingredient"));
           model->setHeaderData(2, Qt::Horizontal, QObject::tr("Quantite d'ingredient"));
           return model;
}



bool Ingredient::modifingredient(int id ,QString nom ,int quantite){
    QSqlQuery query;
    query.prepare("UPDATE ingredient set id=:id,nom=:nom,quantite=:quantite WHERE id=:id");
...
QSqlQueryModel * Ingredient::rechercher (const QString &aux)

{
    QSqlQueryModel * model = new QSqlQueryModel();

    model->setQuery("select * from ingredient where ((id || nom || quantite) LIKE '%"+aux+"%')");
model->setHeaderData(0, Qt::Horizontal, QObject::tr("identifient"));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("Nom d'ingredient"));
  model->setHeaderData(2, Qt::Horizontal, QObject::tr("Quantite d'ingredient"));
